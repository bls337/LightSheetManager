...
/**
 * This frame contains controls for the Acquisition Table.
 */
public class AcquisitionTableFrame extends JFrame {
    ...
    //public AcquisitionTableFrame(final Studio studio, final Prefs prefs, final AcquisitionPanel acqPanel) {
    //public AcquisitionTableFrame(final Studio studio, final AcquisitionPanel acqPanel) {
    public AcquisitionTableFrame(final Studio studio) {
    ...
    private void createUserInterface() {
        ...
        // sub panel for the JList and related elements
        final JPanel listPanel = new JPanel();
        listPanel.setLayout(new MigLayout("", "", ""));

        // sub panel to report acquisition status
        ...
        acqTable_ = new AcquisitionTable(studio_); // new AcquisitionTable(studio_, acqPanel_);
        ...
        // buttons to add and remove acquisitions
...
//        txtErrorLog.setWrapStyleWord(true);
        ...
        DialogUtils.setErrorLog(txtAcqStatusLog_);
    ...
    private void createEventHandlers() {
        ...
        btnAddAcq_.addActionListener(e -> addAcquisition());

        // remove the selected acquisition from the table
        btnRemoveAcq_.addActionListener(e -> acqTable_.removeSelectedAcquisition());

        // save current position list and acquisition settings
        btnSaveAcq_.addActionListener(e -> saveSettings());

        // add a new position list to the JList and HashMap
        btnAddPositionList_.addActionListener(e -> addPositionList());

        // remove a position list from the JList and HashMap
        btnRemovePositionList_.addActionListener(e -> removeSelectedPositionList());

        // rename the selected position list
        btnRenamePositionList_.addActionListener(e -> renameSelectedPositionList());

        // save acquisition settings to a directory
        btnSavePlaylist_.addActionListener(e -> saveAcquisitionTable());

        // open the acquisition settings from a directory
        btnLoadPlaylist_.addActionListener(e -> loadAcquisitionTable());

        // run all acquisitions sequentially
        btnRunAcquisitions_.addItemListener(e -> {
        ...
        //btnEditPositionList_.addActionListener(e -> gui_.showXYPositionList());
        btnEditPositionList_.addActionListener(e -> studio_.app().showPositionList());

        // open the XYZ grid window
        btnOpenGridFrame_.addActionListener(e -> gridFrame_.setVisible(true));

        // browse to the playlist save directory
        btnBrowse_.addActionListener(e -> setSaveDirectory());
    }

    /**
     * Set the XYZ grid frame, so it can be opened it from the playlist.
     *
     * @param frame the XYZ grid frame
     */
    public void setXYZGridFrame(final JFrame frame) {
        gridFrame_ = frame;
    }
...
//                  "The file already exists, would you like to overwrite it?", JOptionPane.YES_NO_OPTION);
    ...
    public void addAcquisition() {
        // get user input
        final String name = DialogUtils.showTextEntryDialog(this,
        ...
        if (acqTable_.getTableData().acqNameExists(name)) {
            ...
            DialogUtils.showErrorMessage(this, "Name Error",
            ...
            acqTable_.addAcquisitionSettings(name, DefaultAcquisitionSettingsDISPIM.builder().build());//acqPanel_.getCurrentAcquisitionSettings());
        }
    }
    ...
    private void renameSelectedPositionList() {
        if (lstPositions_.getNumItems() == 0) {
            DialogUtils.showErrorMessage(this, "Rename Error", "No position list to rename.");
            return;
        }
        // get user input
        ...
        if (oldName.equals(AcquisitionTablePositionList.NO_POSITION_LIST)) {
            DialogUtils.showErrorMessage(this, "Rename Error", "Unable to rename \"None\".");
            return;
        }
        final String newName = DialogUtils.showTextEntryDialog(this,
        ...
        if (lstPositions_.contains(newName)) {
            DialogUtils.showErrorMessage(this,
...
//        DialogUtils.SEND_ERROR_TO_COMPONENT = true;
//
//        // TODO: check this
...
//                DialogUtils.SEND_ERROR_TO_COMPONENT = false;
//
//                // report to user
...
//    }
    ...
    private void saveSettings() {
        ...
        if (acqTable_.getSelectedRow() != -1) {
            ...
            // TODO: check this
            acqTable_.getCurrentAcqName(), DefaultAcquisitionSettingsDISPIM.builder().build());//acqPanel_.getCurrentAcquisitionSettings());
        }
